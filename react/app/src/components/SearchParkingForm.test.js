...
import SearchParkingForm from './SearchParkingForm';
...
test('searching calls the right function', async () => {
    ...
    getFeatures.getParkingById = jest.fn().mockReturnValue([
        {
            position: {
                geometry: {
                    coordinates: [
    ...
    const formField = await screen.findByLabelText('Id på parkering');
    const user = userEvent.setup();
    expect(formField).toBeInTheDocument();
    await user.type(formField, '1');
    const button = await screen.findByText('Sök');
    await user.click(button);
    expect(getFeatures.getParkingById).toHaveBeenCalledTimes(1);
});
